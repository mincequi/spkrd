...
SummedFreqRespPlot::SummedFreqRespPlot() :
	...
	signal_speakerlist_loaded.connect(
			mem_fun(*this, &SummedFreqRespPlot::on_speakerlist_loaded));
	signal_add_crossover_plot.connect(
			mem_fun(*this, &SummedFreqRespPlot::on_add_plot));
	signal_crossover_selected.connect(
			mem_fun(*this, &SummedFreqRespPlot::on_crossover_selected));
}
...
int SummedFreqRespPlot::on_add_plot(vector<GSpeakers::Point>& filter_points,
	...
	int position = -1;
	int l = 0;
	for (vector<int>::iterator iter = m_nets.begin(); iter != m_nets.end();
			++iter) {
		if (*i == *iter) {
			position = l;
		}
