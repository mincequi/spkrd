...
CrossoverWizard::CrossoverWizard() : Gtk::ScrolledWindow(), m_vbox(), m_speaker_list{nullptr}
{
...
{
#ifdef OUTPUT_DEBUG
    std::puts("CrossoverWizard::on_crossover_selected");
#endif
         ...
         ++iter, index++)
    {
        ...
        if (type == CROSSOVER_TYPE_TWOWAY)
        {
            if (index == 0)
            {
                FilterLinkFrame* link1 = new FilterLinkFrame(&(*iter),
        ...
        else if (type == CROSSOVER_TYPE_THREEWAY)
        {
            ...
            {
                FilterLinkFrame* link2 = new FilterLinkFrame(&(*iter),
                                                             _("Midrange filter"),
                                                             m_speaker_list);
            ...
            {
                FilterLinkFrame* link3 = new FilterLinkFrame(&(*iter),
                                                             _("Tweeter filter"),
                                                             m_speaker_list);
        ...
        {
            // FilterLinkFrame* link1 = Gtk::manage(
            //     new FilterLinkFrame(&(*iter), _("filter"), m_speaker_list));
...
{
#ifdef OUTPUT_DEBUG
    std::puts("CrossoverWizard::on_speaker_list_loaded");
#endif
