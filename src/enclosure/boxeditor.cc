...
BoxEditor::BoxEditor()
    : Gtk::Frame(_("Enclosure editor")), m_table(5, 5, true),
      m_speaker_qts_label("", Gtk::ALIGN_START), m_speaker_vas_label("", Gtk::ALIGN_START),
...
void BoxEditor::on_optimize_button_clicked() {
  ...
  if (!disable_signals) {
    ...
    case BOX_TYPE_PORTED:
      ...
      // vb=20*vas*qts^3.3;
      // fb=fs*(vas/vb)^0.31;
      m_box->set_vb1(20 * m_current_speaker.get_vas() * std::pow(m_current_speaker.get_qts(), 3.3));
...
void BoxEditor::on_append_to_plot_clicked() {
  ...
  case BOX_TYPE_PORTED:
    for (int f = 10; f < 1000; f++) {
      ...
      auto const B =
          A / m_current_speaker.get_qts() +
          m_box->get_fb1() / (7.0 * m_current_speaker.get_fs() * m_current_speaker.get_qts());
      auto const C =
          1.0 + A + m_current_speaker.get_vas() / m_box->get_vb1() +
          m_box->get_fb1() / (7.0 * m_current_speaker.get_fs() * m_current_speaker.get_qts());
      auto const D =
          1.0 / m_current_speaker.get_qts() + m_box->get_fb1() / (7.0 * m_current_speaker.get_fs());
      auto const fn2 = std::pow(f / m_current_speaker.get_fs(), 2);
      auto const fn4 = std::pow(fn2, 2);
      point.set_y(10 * std::log10(std::pow(fn4, 2) / (std::pow(fn4 - C * fn2 + A, 2) +
  ...
  case BOX_TYPE_SEALED:
    for (int f = 10; f < 1000; f++) {
      ...
      point.set_y(10 *
                  std::log10(std::pow(fr, 2) / (std::pow(fr - 1.0, 2) + fr / std::pow(qb, 2))));
      points.push_back(point);
    }
    break;
  }
