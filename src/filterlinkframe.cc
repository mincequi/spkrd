...
FilterLinkFrame::FilterLinkFrame(Net *net, const string& description,
		...
		Gtk::Frame(""), adj(1.0, 1.0, 31.0, 1.0, 5.0, 0.0), m_speaker_combo(), m_inv_pol_checkbutton(
				_("Invert polarity"), false), m_damp_spinbutton(
				*(new Gtk::Adjustment(0, 0, 100, 1, 5.0))), m_imp_corr_checkbutton(
				_("Impedance correction")), m_adv_imp_model_checkbutton(
				_("Use adv. driver imp. model")) {
	set_border_width(2);
	set_shadow_type(Gtk::SHADOW_NONE);
	static_cast<Gtk::Label*>(get_label_widget())->set_markup(
			"<b>" + description + "</b>");
	...
	if (m_speaker_list != NULL) {
		for (vector<Speaker>::iterator iter =
	...
	if (m_net->get_has_damp() == true) {
		...
		Speaker speaker;
		if (m_speaker_list != NULL) {
			speaker = m_speaker_list->get_speaker_by_id_string(
					m_speaker_combo.get_entry()->get_text());
		}
		if (g_settings.getValueBool("UseDriverImpedance") == true) {
			m_damp_spinbutton.set_value(
					GSpeakers::round(
							20 * log10(r_ser / speaker.get_imp() + 1)));
		} else {
			m_damp_spinbutton.set_value(
					GSpeakers::round(
							20 * log10(r_ser / speaker.get_rdc() + 1)));
		}
	}
	...
	if (net->get_type() & NET_TYPE_HIGHPASS) {
		...
		static_cast<Gtk::Label*>(frame->get_label_widget())->set_markup(
				"<b>" + Glib::ustring(_("Highpass")) + "</b>");
		...
		menulist.push_back(
				Gtk::Menu_Helpers::MenuElem("1",
						bind<int, int>(
								mem_fun(*this,
										&FilterLinkFrame::on_order_selected), 1,
								1)));
		menulist.push_back(
				Gtk::Menu_Helpers::MenuElem("2",
						bind<int, int>(
								mem_fun(*this,
										&FilterLinkFrame::on_order_selected), 1,
								2)));
		menulist.push_back(
				Gtk::Menu_Helpers::MenuElem("3",
						bind<int, int>(
								mem_fun(*this,
										&FilterLinkFrame::on_order_selected), 1,
								3)));
		menulist.push_back(
				Gtk::Menu_Helpers::MenuElem("4",
						bind<int, int>(
								mem_fun(*this,
										&FilterLinkFrame::on_order_selected), 1,
								4)));
		...
		hbox->pack_start(
				(*manage(new Gtk::Label(_("Order: "), Gtk::ALIGN_LEFT))));
		...
		m_higher_co_freq_spinbutton = manage(
				new Gtk::SpinButton(
						*(new Gtk::Adjustment(2000, 1, 20000, 1, 100))));
		hbox->pack_start(*m_higher_co_freq_spinbutton);
		label = manage(new Gtk::Label(" Hz"));
		label->set_alignment(Gtk::ALIGN_LEFT);
		hbox->pack_start(*label);
		vbox->pack_start(*hbox);
		set_family(m_higher_type_optionmenu, m_net->get_highpass_order(),
	...
	if (m_net->get_type() & NET_TYPE_LOWPASS) {
		...
		menulist.push_back(
				Gtk::Menu_Helpers::MenuElem("1",
						bind<int, int>(
								mem_fun(*this,
										&FilterLinkFrame::on_order_selected), 0,
								1)));
		menulist.push_back(
				Gtk::Menu_Helpers::MenuElem("2",
						bind<int, int>(
		...
		hbox = manage(new Gtk::HBox());
		hbox->pack_start((*manage(new Gtk::Label(_("Cutoff: ")))));
		m_lower_co_freq_spinbutton = manage(
				new Gtk::SpinButton(
						*(new Gtk::Adjustment(2000, 1, 20000, 1, 100))));
		hbox->pack_start(*m_lower_co_freq_spinbutton);
		hbox->pack_start((*manage(new Gtk::Label(_("Hz")))));
		vbox->pack_start(*hbox);
		set_family(m_lower_type_optionmenu, m_net->get_lowpass_order(),
	...
	signal_net_modified_by_user.connect(
			mem_fun(*this, &FilterLinkFrame::on_net_updated));
	signal_plot_crossover.connect(
			mem_fun(*this, &FilterLinkFrame::on_clear_and_plot));
	g_settings.defaultValueString("SPICECmdLine", "gnucap");
	my_filter_plot_index = -1;
	signal_speakerlist_loaded.connect(
			mem_fun(*this, &FilterLinkFrame::on_speakerlist_loaded));
	g_settings.settings_changed.connect(
			mem_fun(*this, &FilterLinkFrame::on_settings_changed));
...
#ifdef OUTPUT_DEBUG
	cout << "FilterLinkFrame::on_order_selected, which = " << which
			<< "   order = " << order << endl;
#endif
...
void FilterLinkFrame::on_param_changed() {
	if (enable_edit == true) {
		...
		if (m_net->get_type() & NET_TYPE_LOWPASS) {
			m_net->set_lowpass_order(
					m_lower_order_optionmenu->get_history() + 1);
			double cutoff = m_lower_co_freq_spinbutton->get_value();
			switch (m_net->get_lowpass_order()) {
			case NET_ORDER_1ST:
				num_params = get_filter_params(NET_BUTTERWORTH, NET_ORDER_1ST,
						NET_TYPE_LOWPASS);
			...
			case NET_ORDER_2ND:
				...
				case 0:  // bessel
					num_params = get_filter_params(NET_BESSEL, NET_ORDER_2ND,
							NET_TYPE_LOWPASS);
					m_net->set_lowpass_family(NET_BESSEL);
					break;
				case 1:  // butterworth
					num_params = get_filter_params(NET_BUTTERWORTH,
							NET_ORDER_2ND, NET_TYPE_LOWPASS);
					m_net->set_lowpass_family(NET_BUTTERWORTH);
					break;
				case 2:  // chebychev
					num_params = get_filter_params(NET_CHEBYCHEV, NET_ORDER_2ND,
							NET_TYPE_LOWPASS);
				...
				/* capacitor */
				(*m_net->parts())[index].set_value(
						(num_params[1] / (speaker.get_rdc() * cutoff))
								* 1000000);
			...
			case NET_ORDER_3RD:
				switch (m_lower_type_optionmenu->get_history()) {
				case 0:   // bessel
					num_params = get_filter_params(NET_BESSEL, NET_ORDER_3RD,
							NET_TYPE_LOWPASS);
					m_net->set_lowpass_family(NET_BESSEL);
					break;
				case 1:   // butterworth
					num_params = get_filter_params(NET_BUTTERWORTH,
							NET_ORDER_3RD, NET_TYPE_LOWPASS);
					m_net->set_lowpass_family(NET_BUTTERWORTH);
					break;
				}
				/* inductor */
				...
				/* inductor */
				(*m_net->parts())[index].set_value(
						((speaker.get_rdc() * num_params[2]) / cutoff) * 1000);
				(*m_net->parts())[index++].set_unit("m");
				break;
			case NET_ORDER_4TH:
				switch (m_lower_type_optionmenu->get_history()) {
				case 0:  // bessel
					num_params = get_filter_params(NET_BESSEL, NET_ORDER_4TH,
							NET_TYPE_LOWPASS);
					m_net->set_lowpass_family(NET_BESSEL);
					break;
				case 1:  // butterworth
					num_params = get_filter_params(NET_BUTTERWORTH,
							NET_ORDER_4TH, NET_TYPE_LOWPASS);
					m_net->set_lowpass_family(NET_BUTTERWORTH);
					break;
				case 2:  // gaussian
					num_params = get_filter_params(NET_GAUSSIAN, NET_ORDER_4TH,
							NET_TYPE_LOWPASS);
					m_net->set_lowpass_family(NET_GAUSSIAN);
					break;
				case 3:  // legendre
					num_params = get_filter_params(NET_LEGENDRE, NET_ORDER_4TH,
							NET_TYPE_LOWPASS);
				...
				/* capacitor */
				(*m_net->parts())[index].set_value(
						(num_params[3] / (speaker.get_rdc() * cutoff))
								* 1000000);
				(*m_net->parts())[index++].set_unit("u");
				break;
			}
		...
		if (m_net->get_type() & NET_TYPE_HIGHPASS) {
			m_net->set_highpass_order(
					m_higher_order_optionmenu->get_history() + 1);
			...
			case NET_ORDER_2ND:
				...
				case 1:  // butterworth
					num_params = get_filter_params(NET_BUTTERWORTH,
							NET_ORDER_2ND, NET_TYPE_HIGHPASS);
					m_net->set_highpass_family(NET_BUTTERWORTH);
					break;
				case 2:  // chebychev
					num_params = get_filter_params(NET_CHEBYCHEV, NET_ORDER_2ND,
				...
				/* capacitor */
				(*m_net->parts())[index].set_value(
						(num_params[0] / (speaker.get_rdc() * cutoff))
								* 1000000);
				(*m_net->parts())[index++].set_unit("u");
				/* inductor */
				(*m_net->parts())[index].set_value(
			...
			case NET_ORDER_3RD:
				...
				case 1:   // butterworth
					num_params = get_filter_params(NET_BUTTERWORTH,
							NET_ORDER_3RD, NET_TYPE_HIGHPASS);
					m_net->set_highpass_family(NET_BUTTERWORTH);
					break;
				}
				/* capacitor */
				...
				/* capacitor */
				(*m_net->parts())[index].set_value(
						(num_params[2] / (speaker.get_rdc() * cutoff))
								* 1000000);
				(*m_net->parts())[index++].set_unit("u");
				break;
			case NET_ORDER_4TH:
				...
				case 1:  // butterworth
					num_params = get_filter_params(NET_BUTTERWORTH,
							NET_ORDER_4TH, NET_TYPE_HIGHPASS);
					m_net->set_highpass_family(NET_BUTTERWORTH);
					break;
				case 2:  // gaussian
					num_params = get_filter_params(NET_GAUSSIAN, NET_ORDER_4TH,
		...
		if (m_imp_corr_checkbutton.get_active() == true) {
			...
			/* calc imp corr here */
			m_net->get_imp_corr_C().set_value(
					(speaker.get_lvc() / 1000) / pow(speaker.get_rdc(), 2)
							* 1000000);
		...
		if (m_damp_spinbutton.get_value_as_int() > 0) {
			...
			double r_ser = speaker.get_rdc()
					* (pow(10, m_damp_spinbutton.get_value() / 20) - 1);
			double r_par = speaker.get_rdc()
					+ pow(speaker.get_rdc(), 2) / r_ser;
...
void FilterLinkFrame::on_plot_crossover() {
...
#ifdef OUTPUT_DEBUG
		cout << "FilterLinkFrame::on_plot_crossover: ERROR: " << e.what()
				<< endl;
#endif
		Gtk::MessageDialog d(
				_("FilterLinkFrame::on_plot_crossover: ERROR: ")
						+ Glib::ustring(e.what()), false, Gtk::MESSAGE_ERROR,
				Gtk::BUTTONS_OK, true);
		d.run();
	}
	...
	if ((g_settings.getValueBool("SPICEUseNGSPICE") == true)
		...
		cmd = g_settings.getValueString("SPICECmdLine") + " -b "
				+ spice_filename + " > " + spice_filename + ".out";
	} else {
		cmd = g_settings.getValueString("SPICECmdLine") + " -b -o "
				+ spice_filename + ".out " + spice_filename;
	}
#ifdef OUTPUT_DEBUG
	cout
			<< "FilterLinkFrame::on_plot_crossover: running SPICE with \"" + cmd
					+ "\"" << endl;
#endif
	system(cmd.c_str());
#ifdef OUTPUT_DEBUG
...
// 	    }
		...
		if (enable_edit == true) {
			...
			if (m_net->get_type() & NET_TYPE_LOWPASS) {
				...
				points[index1 + 1].set_y(
						points[index1 + 1].get_y()
								+ m_damp_spinbutton.get_value());
				points[index1].set_y(
						points[index1].get_y() + m_damp_spinbutton.get_value());

				double ydiff = points[index1 + 1].get_y()
						- points[index1].get_y();
				int xdiff = points[index1 + 1].get_x() - points[index1].get_x();
				double ytodbdiff = points[index1].get_y() + 3;
				m_lower_co_freq_spinbutton->set_value(
						(ytodbdiff / ydiff) * xdiff
			...
			if (m_net->get_type() & NET_TYPE_HIGHPASS) {
				...
				index2++;
				points[index2 - 1].set_y(
						points[index2 - 1].get_y()
								+ m_damp_spinbutton.get_value());
				points[index2].set_y(
						points[index2].get_y() + m_damp_spinbutton.get_value());

				double ydiff = points[index2 - 1].get_y()
						- points[index2].get_y();
				int xdiff = points[index2].get_x() - points[index2 - 1].get_x();
				double ytodbdiff = points[index2].get_y() + 3;
				m_higher_co_freq_spinbutton->set_value(
						(ytodbdiff / ydiff) * xdiff + points[index2].get_x());
			}
			enable_edit = true;
		}
	}
 ...
 */
vector<double> FilterLinkFrame::get_filter_params(int net_name_type,
		int net_order, int net_type) {
	vector<double> nums;
	switch (net_order) {
	case NET_ORDER_1ST:
		switch (net_type) {
...
void FilterLinkFrame::set_family(Gtk::OptionMenu *option_menu, int order,
		int family) {
#ifdef OUTPUT_DEBUG
	cout << "FilterLinkFrame::set_family: order = " << order << ", family = "
			<< family << endl;
#endif
	switch (order) {
	case NET_ORDER_2ND:
