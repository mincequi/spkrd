...
Speaker_ListStore::Speaker_ListStore()
  ...
  m_Table.attach(*Gtk::manage(new Gtk::Label(_("Sensitivity: (dB/W/m)"), Gtk::ALIGN_START)), 0, 2,
                 9, 10);
  m_Table.attach(m_SensEntry, 2, 3, 9, 10);

  m_Table.attach(*Gtk::manage(new Gtk::Label(_("Cone mass: (kg)"), Gtk::ALIGN_START)), 0, 2, 10,
                 11);
  m_Table.attach(m_MmdEntry, 2, 3, 10, 11);
  m_Table.attach(*Gtk::manage(new Gtk::Label(_("Effective radius: (m)"), Gtk::ALIGN_START)), 0, 2,
                 11, 12);
  m_Table.attach(m_AdEntry, 2, 3, 11, 12);
  m_Table.attach(*Gtk::manage(new Gtk::Label(_("Force factor: (N/A)"), Gtk::ALIGN_START)), 0, 2, 12,
                 13);
  m_Table.attach(m_BlEntry, 2, 3, 12, 13);
  m_Table.attach(*Gtk::manage(new Gtk::Label(_("Susp. resistance: (Ns/m)"), Gtk::ALIGN_START)), 0,
                 2, 13, 14);
  m_Table.attach(m_RmsEntry, 2, 3, 13, 14);
  m_Table.attach(*Gtk::manage(new Gtk::Label(_("Susp. compleance: (m/N)"), Gtk::ALIGN_START)), 0, 2,
                 14, 15);
  ...
  // m_Table.attach(*Gtk::manage(new Gtk::Label(_("Freq resp file:"), Gtk::ALIGN_START)), 0, 1, 18,
  // 19); m_Table.attach(m_FreqRespFileEntry, 1, 2, 18, 19);
  ...
  m_EditFreqRespButton.signal_clicked().connect(
      sigc::mem_fun(*this, &Speaker_ListStore::on_edit_freq_resp));
  m_BrowseFreqRespButton.signal_clicked().connect(
      sigc::mem_fun(*this, &Speaker_ListStore::on_browse_freq_resp));
  ...
  signal_save_open_files.connect(sigc::mem_fun(*this, &Speaker_ListStore::on_save_open_files));
}
...
Gtk::Menu& Speaker_ListStore::get_menu() {
  ...
  //     _("_New Driver"), *im, sigc::mem_fun(*this, &Speaker_ListStore::on_new)));
  // menulist.push_back(Gtk::Menu_Helpers::SeparatorElem());
  //
  // im = Gtk::manage(new Gtk::Image(Gtk::Stock::NEW, Gtk::ICON_SIZE_MENU));
  // menulist.push_back(Gtk::Menu_Helpers::ImageMenuElem(
  //     _("N_ew"), *im, sigc::mem_fun(*this, &Speaker_ListStore::on_new_xml)));
  //
  // menulist.push_back(Gtk::Menu_Helpers::MenuElem(
  //     _("A_ppend Driver Xml..."), sigc::mem_fun(*this, &Speaker_ListStore::on_append_xml)));
  //
  // im = Gtk::manage(new Gtk::Image(Gtk::Stock::OPEN, Gtk::ICON_SIZE_MENU));
  // menulist.push_back(Gtk::Menu_Helpers::ImageMenuElem(
  //     _("_Open..."), *im, sigc::mem_fun(*this, &Speaker_ListStore::on_open_xml)));
  //
  // menulist.push_back(Gtk::Menu_Helpers::SeparatorElem());
  //
  // im = Gtk::manage(new Gtk::Image(Gtk::Stock::SAVE, Gtk::ICON_SIZE_MENU));
  // menulist.push_back(Gtk::Menu_Helpers::ImageMenuElem(
  //     _("_Save"), *im, sigc::mem_fun(*this, &Speaker_ListStore::on_save)));
  //
  // im = Gtk::manage(new Gtk::Image(Gtk::Stock::SAVE_AS, Gtk::ICON_SIZE_MENU));
  // menulist.push_back(Gtk::Menu_Helpers::ImageMenuElem(
  //     _("Save _As..."), *im, sigc::mem_fun(*this, &Speaker_ListStore::on_save_as)));
  //
  // menulist.push_back(Gtk::Menu_Helpers::SeparatorElem());
  //
  // im = Gtk::manage(new Gtk::Image(Gtk::Stock::DELETE, Gtk::ICON_SIZE_MENU));
  // menulist.push_back(Gtk::Menu_Helpers::ImageMenuElem(
  //     _("Delete"), *im, sigc::mem_fun(*this, &Speaker_ListStore::on_remove)));
  //
...
Gtk::Widget& Speaker_ListStore::get_toolbar() {
  if (tbar == nullptr) {
    ...
    t->signal_clicked().connect(sigc::mem_fun(*this, &Speaker_ListStore::on_new));
    tbar->append(*t);
    ...
    t->signal_clicked().connect(sigc::mem_fun(*this, &Speaker_ListStore::on_open_xml));
    tbar->append(*t);
    t = Gtk::manage(new Gtk::ToolButton(Gtk::Stock::SAVE));
    t->signal_clicked().connect(sigc::mem_fun(*this, &Speaker_ListStore::on_save));
    tbar->append(*t);
    ...
    t->signal_clicked().connect(sigc::mem_fun(*this, &Speaker_ListStore::on_remove));
    tbar->append(*t);
...
void Speaker_ListStore::on_settings_changed(const std::string& setting) {
...
void Speaker_ListStore::set_entries_sensitive(bool const is_sensitive) {
...
void Speaker_ListStore::on_new() {
  ...
  Gtk::TreePath path(std::to_string(m_speaker_list->speaker_list().size() - 1));
...
void Speaker_ListStore::on_save_as() {
  ...
  std::string const& filename =
      GSpeakersFileChooserDialog(_("Save speaker xml as"), Gtk::FILE_CHOOSER_ACTION_SAVE,
                                 m_filename)
          .get_filename();
        ...
        if (!s.get_freq_resp_filename().empty()) {
          ...
          if (fin.good()) {
            ...
            while (!fin.eof()) {
              ...
              float const f1 = g_ascii_strtod(substr_ptr, nullptr);
  ...
  if (g_settings.getValueBool("DrawDriverImpPlot")) {
    ...
    if (of.good()) {
      ...
      of << "R" << s.get_id() << " 1 2 " << g_ascii_dtostr(buffer.data(), 8, s.get_rdc())
         << std::endl;
      of << "L" << s.get_id() << " 2 3 " << g_ascii_dtostr(buffer.data(), 8, s.get_lvc()) << "mH"
         << std::endl;
      of << "lces 3 0 " << g_ascii_dtostr(buffer.data(), 8, lces) << "mH" << std::endl;
      of << "cmes 3 0 " << g_ascii_dtostr(buffer.data(), 8, cmes) << "uF" << std::endl;
      of << "res 3 0 " << g_ascii_dtostr(buffer.data(), 8, res) << std::endl;
      of << "cmef 3 0 " << g_ascii_dtostr(buffer.data(), 8, cmef) << "uF" << std::endl;
      ...
      if (fin.good()) {
        ...
        Gdk::Color c2("red");
        if (update) {
          int i = s.get_freq_resp_filename().empty() ||
...
void Speaker_ListStore::on_clear() {
  m_refListStore->clear();
  m_speaker_list->speaker_list().clear();
  m_modified = true;
}
  ...
  if (!updating_entries) {
    ...
    if (Gtk::TreeIter iter = refSelection->get_selected()) {
      ...
      case 0:
        ...
        m_speaker_list->speaker_list()[index].set_id_string(m_IdStringEntry.get_text());
        break;
      case 1:
        row[m_columns.qts] = std::atof(m_QtsEntry.get_text().c_str()); // the treestore
        m_speaker_list->speaker_list()[index].set_qts(std::atof(m_QtsEntry.get_text().c_str()));
        break;
      case 2:
        row[m_columns.fs] = std::atof(m_FsEntry.get_text().c_str()); // the treestore
        m_speaker_list->speaker_list()[index].set_fs(std::atof(m_FsEntry.get_text().c_str()));
        break;
      case 3:
        row[m_columns.vas] = std::atof(m_VasEntry.get_text().c_str()); // the treestore
        m_speaker_list->speaker_list()[index].set_vas(std::atof(m_VasEntry.get_text().c_str()));
        break;
      case 4:
        ...
        if (d == 0.0)
          d = 1.0;
        m_speaker_list->speaker_list()[index].set_rdc(d);
      ...
      case 5:
        ...
        if (d == 0.0)
          d = 1.0;
        m_speaker_list->speaker_list()[index].set_lvc(d);
        break;
        update_imp_plot = true;
      case 6:
        row[m_columns.qms] = std::atof(m_QmsEntry.get_text().c_str()); // the treestore
        m_speaker_list->speaker_list()[index].set_qms(std::atof(m_QmsEntry.get_text().c_str()));
        break;
      case 7:
        row[m_columns.qes] = std::atof(m_QesEntry.get_text().c_str()); // the treestore
        m_speaker_list->speaker_list()[index].set_qes(std::atof(m_QesEntry.get_text().c_str()));
        break;
      case 8:
        row[m_columns.imp] = std::atof(m_ImpEntry.get_text().c_str()); // the treestore
        m_speaker_list->speaker_list()[index].set_imp(std::atof(m_ImpEntry.get_text().c_str()));
        break;
      case 9:
        row[m_columns.sens] = std::atof(m_SensEntry.get_text().c_str()); // the treestore
        m_speaker_list->speaker_list()[index].set_sens(std::atof(m_SensEntry.get_text().c_str()));
        break;
      case 10:
      ...
      case 13:
        ...
        if (d == 0.0)
          d = 1.0;
        m_speaker_list->speaker_list()[index].set_mmd(d);
        update_imp_plot = true;
        break;
      case 14:
        ...
        if (d == 0.0)
          d = 1.0;
        m_speaker_list->speaker_list()[index].set_ad(d);
        update_imp_plot = true;
        break;
      case 15:
        ...
        if (d == 0.0)
          d = 1.0;
        m_speaker_list->speaker_list()[index].set_bl(d);
        update_imp_plot = true;
        break;
      case 16:
        ...
        if (d == 0.0)
          d = 1.0;
        m_speaker_list->speaker_list()[index].set_rms(d);
        update_imp_plot = true;
        break;
      case 17:
        ...
        if (d == 0.0)
          d = 1.0;
        m_speaker_list->speaker_list()[index].set_cms(d);
        update_imp_plot = true;
        break;
      }
      if (update_imp_plot) {
...
void Speaker_ListStore::append_xml(const std::string& filename) {
  try {
    ...
    std::for_each(begin(temp_speaker_list.speaker_list()), end(temp_speaker_list.speaker_list()),
                  sigc::mem_fun(*this, &Speaker_ListStore::liststore_add_item));
...
bool Speaker_ListStore::open_xml(const std::string& filename) {
  if (!filename.empty()) {
    try {
      ...
      // GSpeakers::tooltips().set_tip(*m_evbox, m_filename);
      std::for_each(temp_speaker_list.speaker_list().begin(),
                    temp_speaker_list.speaker_list().end(),
                    sigc::mem_fun(*this, &Speaker_ListStore::liststore_add_item));

      m_speaker_list->speaker_list().clear();
      m_speaker_list->speaker_list().insert(begin(m_speaker_list->speaker_list()),
                                            begin(temp_speaker_list.speaker_list()),
                                            end(temp_speaker_list.speaker_list()));
...
void Speaker_ListStore::create_model() {
  ...
  std::for_each(begin(m_speaker_list->speaker_list()), end(m_speaker_list->speaker_list()),
                sigc::mem_fun(*this, &Speaker_ListStore::liststore_add_item));
}
...
void Speaker_ListStore::add_columns() {
  ...
  }
  {
    ...
    pColumn->set_cell_data_func(*pRenderer,
                                sigc::mem_fun(*this, &Speaker_ListStore::type_cell_data_func));
  }
  {
    Gtk::CellRendererText* pRenderer = Gtk::manage(new Gtk::CellRendererText());
    ...
    pColumn->set_cell_data_func(*pRenderer,
                                sigc::mem_fun(*this, &Speaker_ListStore::qts_cell_data_func));
  }
  {
    Gtk::CellRendererText* pRenderer = Gtk::manage(new Gtk::CellRendererText());
    ...
    pColumn->set_cell_data_func(*pRenderer,
                                sigc::mem_fun(*this, &Speaker_ListStore::fs_cell_data_func));
  }
  {
    Gtk::CellRendererText* pRenderer = Gtk::manage(new Gtk::CellRendererText());
    ...
    pColumn->set_cell_data_func(*pRenderer,
                                sigc::mem_fun(*this, &Speaker_ListStore::vas_cell_data_func));
  }
  {
    Gtk::CellRendererText* pRenderer = Gtk::manage(new Gtk::CellRendererText());
    ...
    pColumn->set_cell_data_func(*pRenderer,
                                sigc::mem_fun(*this, &Speaker_ListStore::imp_cell_data_func));
  }
  {
    Gtk::CellRendererText* pRenderer = Gtk::manage(new Gtk::CellRendererText());
    ...
    pColumn->set_cell_data_func(*pRenderer,
                                sigc::mem_fun(*this, &Speaker_ListStore::sens_cell_data_func));
  }
}
