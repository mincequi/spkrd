...
std::string Net::to_SPICE(Speaker& s, bool use_gnucap) {
 ...
 std::string tmp_file = tmp_dir + "/net" + GSpeakers::int_to_ustring(m_id) + ".tmp";
  ...
  if (of.good()) {
    of << "Crossover network SPICE code generated by GSpeakers Redux-" << VERSION << endl;
    of << "vamp " << ++node_counter << " " << 0 << " dc 0 ac 1" << endl;
    if (m_lowpass_order > 0) {
      ...
      case NET_ORDER_1ST:
        ...
        of << "L" << m_parts[part_index].get_id() << " " << node_counter << " " << node_counter - 1
           ...
           << m_parts[part_index].get_unit() << endl;
        part_index++;
        break;
      case NET_ORDER_2ND:
        node_counter++;
...
void Net::set_speaker(string speaker) { m_speaker = speaker; }
