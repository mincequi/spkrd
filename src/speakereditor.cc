...
Speaker_ListStore::Speaker_ListStore() :
		m_TreeViewTable(10, 4, true), m_Table(19, 2, true), m_EditFreqRespButton(
				_("Edit...")), m_BrowseFreqRespButton(_("...")), m_BassCheckButton(
				_("Bass")), m_MidrangeCheckButton(_("Midrange")), m_TweeterCheckButton(
				_("Tweeter")), m_treeview_frame("") {
	...
	g_settings.defaultValueString("SpeakerListXml",
			string(GSPEAKERS_PREFIX) + "/share/xml/vifa.xml");
#endif
	...
	m_frame_label->set_markup(
			"<b>" + Glib::ustring(_("Drivers ["))
...
//                               GSpeakers::short_filename(m_filename) + "]</b>");
	...
	m_Table.attach(*manage(new Gtk::Label(_("Name:"), Gtk::ALIGN_LEFT)), 0, 1,
			0, 1);
	...
	m_Table.attach(*manage(new Gtk::Label(_("Qts:"), Gtk::ALIGN_LEFT)), 0, 2, 1,
			2, Gtk::FILL | Gtk::EXPAND | Gtk::SHRINK);
	m_Table.attach(m_QtsEntry, 2, 3, 1, 2);
	m_Table.attach(*manage(new Gtk::Label(_("Fs: (Hz)"), Gtk::ALIGN_LEFT)), 0,
			2, 2, 3);
	m_Table.attach(m_FsEntry, 2, 3, 2, 3);
	m_Table.attach(*manage(new Gtk::Label(_("Vas: (l)"), Gtk::ALIGN_LEFT)), 0,
			2, 3, 4);
	m_Table.attach(m_VasEntry, 2, 3, 3, 4);
	m_Table.attach(*manage(new Gtk::Label(_("Rdc: (Ohm)"), Gtk::ALIGN_LEFT)), 0,
			2, 4, 5);
	m_Table.attach(m_RdcEntry, 2, 3, 4, 5);
	m_Table.attach(*manage(new Gtk::Label(_("Lvc: (mH)"), Gtk::ALIGN_LEFT)), 0,
			2, 5, 6);
	m_Table.attach(m_LvcEntry, 2, 3, 5, 6);
	m_Table.attach(*manage(new Gtk::Label(_("Qms:"), Gtk::ALIGN_LEFT)), 0, 2, 6,
			7);
	m_Table.attach(m_QmsEntry, 2, 3, 6, 7);
	m_Table.attach(*manage(new Gtk::Label(_("Qes:"), Gtk::ALIGN_LEFT)), 0, 2, 7,
			8);
	m_Table.attach(m_QesEntry, 2, 3, 7, 8);
	m_Table.attach(
			*manage(new Gtk::Label(_("Impedance: (Ohm)"), Gtk::ALIGN_LEFT)), 0,
			2, 8, 9);
	m_Table.attach(m_ImpEntry, 2, 3, 8, 9);
	m_Table.attach(
			*manage(
					new Gtk::Label(_("Sensitivity: (dB/W/m)"),
							Gtk::ALIGN_LEFT)), 0, 2, 9, 10);
	m_Table.attach(m_SensEntry, 2, 3, 9, 10);

	m_Table.attach(
			*manage(new Gtk::Label(_("Cone mass: (kg)"), Gtk::ALIGN_LEFT)), 0,
			2, 10, 11);
	m_Table.attach(m_MmdEntry, 2, 3, 10, 11);
	m_Table.attach(
			*manage(
					new Gtk::Label(_("Effective radius: (m)"),
							Gtk::ALIGN_LEFT)), 0, 2, 11, 12);
	m_Table.attach(m_AdEntry, 2, 3, 11, 12);
	m_Table.attach(
			*manage(new Gtk::Label(_("Force factor: (N/A)"), Gtk::ALIGN_LEFT)),
			0, 2, 12, 13);
	m_Table.attach(m_BlEntry, 2, 3, 12, 13);
	m_Table.attach(
			*manage(
					new Gtk::Label(_("Susp. resistance: (Ns/m)"),
							Gtk::ALIGN_LEFT)), 0, 2, 13, 14);
	m_Table.attach(m_RmsEntry, 2, 3, 13, 14);
	m_Table.attach(
			*manage(
					new Gtk::Label(_("Susp. compleance: (m/N)"),
							Gtk::ALIGN_LEFT)), 0, 2, 14, 15);
	m_Table.attach(m_CmsEntry, 2, 3, 14, 15);

	GSpeakers::tooltips().set_tip(m_BassCheckButton,
			_(
					"Check this box if the driver will work as a woofer (bass speaker)"));
	...
	vbox->pack_start(
			*manage(new Gtk::Label(_("Freq resp file:"), Gtk::ALIGN_LEFT)));
	...
	m_MmdEntry.signal_changed().connect(
			bind<int>(mem_fun(*this, &Speaker_ListStore::on_entry_changed),
					13));
	...
	GSpeakers::tooltips().set_tip(m_EditFreqRespButton,
			_("Edit frequency response for this driver"));
	GSpeakers::tooltips().set_tip(m_BrowseFreqRespButton,
			_("Browse for frequnecy response file"));
	...
	selection->signal_changed().connect(
			mem_fun(*this, &Speaker_ListStore::on_selection_changed));
	//selection->set_mode(Gtk::SELECTION_SINGLE);

	g_settings.settings_changed.connect(
			mem_fun(*this, &Speaker_ListStore::on_settings_changed));
	...
	signal_save_open_files.connect(
			mem_fun(*this, &Speaker_ListStore::on_save_open_files));
...
Gtk::Menu& Speaker_ListStore::get_menu() {
	...
	menulist.push_back(
			Gtk::Menu_Helpers::ImageMenuElem(_("Delete"), *im,
...
Gtk::Widget& Speaker_ListStore::get_toolbar() {
	if (tbar == NULL) {
		...
		// TODO: tooltips
		Gtk::Widget *im = manage(
				new Gtk::Image(Gtk::Stock::NEW, Gtk::ICON_SIZE_LARGE_TOOLBAR));
		...
		toolbar.add(*tbar);
		tbar->set_toolbar_style(
				(Gtk::ToolbarStyle) g_settings.getValueUnsignedInt(
						"ToolbarStyle"));
...
void Speaker_ListStore::on_new() {
	...
	s.set_id_string(
			s.get_id_string() + " " + GSpeakers::int_to_ustring(s.get_id()));
	...
	if (const Gtk::TreeIter iter = refSelection->get_selected()) {
		...
		if (indices.size() > 0) {
			...
			if (index < (int) m_speaker_list->speaker_list()->size())
				m_speaker_list->speaker_list()->erase(
						m_speaker_list->speaker_list()->begin() + index);
		}
	}
...
void Speaker_ListStore::on_save_as() {
	GSpeakersFileChooserDialog *fc = new GSpeakersFileChooserDialog(
			_("Save speaker xml as"), Gtk::FILE_CHOOSER_ACTION_SAVE,
			m_filename);
...
void Speaker_ListStore::on_selection_changed() {
	...
	if (const Gtk::TreeIter iter = refSelection->get_selected()) {
		...
		if (indices.size() > 0) {
			...
			GSpeakers::tooltips().set_tip(m_FreqRespFileEntry,
			...
			if (g_settings.getValueBool("DrawDriverFreqRespPlot") == true) {
				...
				if (s.get_freq_resp_filename() != "") {
					...
					if (fin.good()) {
						...
						cout << "Could not open " << s.get_freq_resp_filename()
								<< endl;
					}
				}
			}
...
#else
		string tmp_file = Glib::get_tmp_dir() + "/speaker"
				+ GSpeakers::int_to_ustring(s.get_id()) + ".tmp";
#endif

		ofstream of(tmp_file.c_str());
		if (of.good()) {
			of << "SPICE code generated by GSpeakers Redux-" << VERSION << endl;
			of << "vamp 1 0 dc 0 ac 1" << endl;
			...
			double cmef = 8 * po * s.get_ad() * s.get_ad() * s.get_ad()
					/ (3 * s.get_bl() * s.get_bl()) * 1000000;
			of << "R" << s.get_id() << " 1 2 "
					<< g_ascii_dtostr(buffer, 8, s.get_rdc()) << endl;
			of << "L" << s.get_id() << " 2 3 "
					<< g_ascii_dtostr(buffer, 8, s.get_lvc()) << "mH" << endl;
			of << "lces 3 0 " << g_ascii_dtostr(buffer, 8, lces) << "mH"
					<< endl;
			of << "cmes 3 0 " << g_ascii_dtostr(buffer, 8, cmes) << "uF"
					<< endl;
			of << "res 3 0 " << g_ascii_dtostr(buffer, 8, res) << endl;
			of << "cmef 3 0 " << g_ascii_dtostr(buffer, 8, cmef) << "uF"
					<< endl;
			...
			if ((g_settings.getValueBool("SPICEUseNGSPICE")) == true
				...
				cmd = g_settings.getValueString("SPICECmdLine") + " -b "
						+ tmp_file + " > " + tmp_file + ".out";
			} else {
				cmd = g_settings.getValueString("SPICECmdLine") + " -b -o "
						+ tmp_file + ".out " + tmp_file;
			}
			//g_settings.getValueString("SPICECmdLine") + " -b -o " + tmp_file + ".out " + tmp_file;
#ifdef OUTPUT_DEBUG
			cout
					<< "Speaker_ListStore::draw_imp_plot: running SPICE with \""
							+ cmd + "\"" << endl;
#endif
			system(cmd.c_str());
#ifdef OUTPUT_DEBUG
			...
			if (fin.good()) {
				...
				if (update == true) {
					int i;
					if ((s.get_freq_resp_filename() == "")
							|| !(g_settings.getValueBool(
									"DrawDriverFreqRespPlot"))) {
						i = 0;
					} else {
						i = 1;
					}
...
void Speaker_ListStore::on_clear() {
	m_refListStore->clear();
	m_speaker_list->speaker_list()->erase(
			m_speaker_list->speaker_list()->begin(),
	...
	if (updating_entries == false) {
		...
		if (Gtk::TreeIter iter = refSelection->get_selected()) {
			...
			case 10:
				...
				row[m_columns.type] =
						(*(m_speaker_list->speaker_list()))[index].get_type();
				signal_speakerlist_loaded(m_speaker_list);
				break;
			case 11:
				//cout << "midrange" << endl;
...
void Speaker_ListStore::append_xml(const std::string& filename) {
	...
	try {
		...
		for_each(temp_speaker_list.speaker_list()->begin(),
				temp_speaker_list.speaker_list()->end(),
				mem_fun(*this, &Speaker_ListStore::liststore_add_item));
		for (vector<Speaker>::iterator from =
				temp_speaker_list.speaker_list()->begin();
...
bool Speaker_ListStore::open_xml(const std::string& filename) {
	...
	if (filename.length() > 0) {
		try {
			...
			m_frame_label->set_markup(
					"<b>" + Glib::ustring(_("Drivers ["))
							+ GSpeakers::short_filename(m_filename, 40)
							+ "]</b>");
			GSpeakers::tooltips().set_tip(*m_evbox, m_filename);
			for_each(temp_speaker_list.speaker_list()->begin(),
					temp_speaker_list.speaker_list()->end(),
...
void Speaker_ListStore::on_edit_freq_resp() {
	...
	cout << "SpeakerEditor::on_edit_freq_resp: index = " << index << endl;
#endif
	...
	(*m_speaker_list->speaker_list())[index].set_freq_resp_filename(
			f->get_filename());
	delete f;
	on_selection_changed();
	m_menu.items()[MENU_INDEX_SAVE].set_sensitive(true);
...
#endif
	GSpeakersFileChooserDialog *fc = new GSpeakersFileChooserDialog(
			_("Open frequency response file"));
	std::string filename = fc->get_filename();
...
void Speaker_ListStore::create_model() {
	...
	for_each(m_speaker_list->speaker_list()->begin(),
			m_speaker_list->speaker_list()->end(),
			mem_fun(*this, &Speaker_ListStore::liststore_add_item));
}
...
void Speaker_ListStore::add_columns() {
	 ...
	 */
	{
		Gtk::CellRendererText* pRenderer = Gtk::manage(
	...
	}
	{
		...
		pColumn->set_cell_data_func(*pRenderer,
				mem_fun(*this, &Speaker_ListStore::qts_cell_data_func));
	...
	}
	{
		...
		pColumn->set_cell_data_func(*pRenderer,
				mem_fun(*this, &Speaker_ListStore::fs_cell_data_func));
	...
	}
	{
		...
		pColumn->set_cell_data_func(*pRenderer,
				mem_fun(*this, &Speaker_ListStore::vas_cell_data_func));
...
void Speaker_ListStore::qts_cell_data_func(Gtk::CellRenderer *cell,
	...
	Gtk::CellRendererText& renderer =
			dynamic_cast<Gtk::CellRendererText&>(*cell);
	renderer.property_text() = GSpeakers::double_to_ustring(
			(*iter)[m_columns.qts], 3, 3);
...
void Speaker_ListStore::cms_cell_data_func(Gtk::CellRenderer *cell,
	...
	Gtk::CellRendererText& renderer =
			dynamic_cast<Gtk::CellRendererText&>(*cell);
	renderer.property_text() = GSpeakers::double_to_ustring(
			(*iter)[m_columns.cms], 3, 4) + " m/N";
	renderer.property_xalign() = 1.0;
}

void Speaker_ListStore::on_cell_fixed_toggled(
